...
async function enviarOneSignal(titulo, mensagem) {
...
async function analisar(symbol) {
  try {
    ...
    const ema200 = EMA(200, closeM5);

    const rsiAtual = RSI(14, closeM5.slice(-15));
    const rsiAnterior = RSI(14, closeM5.slice(-16, -1));
    ...
    if (dir) {
      ...
      await enviarOneSignal("SNIPER PRO", msg);
    }
